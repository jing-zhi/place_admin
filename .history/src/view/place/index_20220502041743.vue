...
import { getPlaceList,createPlace,setInfo,deletePlace } from '@/api/place.js'
...
const switchChange = (row) => {
    ...
    console.log({ id: row.cd_id,status:row.qyzt })
    // 启用
    // setStatus(data).then(() => {
    // })
...
const deletePlaceFun = async(row) => {
  console.log(row.cd_id)
  const res = await deletePlace({ id: row.status })
    if (res.code === 0) {
