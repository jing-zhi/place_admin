  ...
  <div>
    ...
    >
      <div style="height:60vh;overflow:auto;padding:0 10px;">
        <el-form ref="placeForm"  :rules="rules" :model="placeInfo" label-width="100px">
...
const options = [
        ...
        { label: "公共浴室", code: 29 },
        { label: "医疗机构", code: 30 },
        // { label: "铁路客运（无）", code: 31 },
        { label: "道路客运", code: 32 },
        // { label: "水陆客运（无）", code: 33 },
        // { label: "民航（无）", code: 34 },
        { label: "城市公共汽电车", code: 35 },
        // { label: "城市轨道交通（无）", code: 36 },
        { label: "出租汽车", code: 37 },
        // { label: "回国人员转运车辆（无）", code: 38 },
...
const switchChange = async(row) => {
    ...
    // const data = { id: row.cd_id,status:row.qyzt }
    // console.log(data)
    console.log({ id: row.id,status:row.status })
    // 启用
    // setStatus({ id: 4,status:row.status }).then(() => {
    //     if (resStatus.code === 0) {
    //             console.log("yy")
